...
public class ResizeImage {
    ...
    public BufferedImage zoomImage(BufferedImage im, float resizeTimes) {
        ...
        /*调整后的图片的宽度和高度*/
		/*
		 * int toWidth=840; int toHeight=560;
		 */
        /*原始图像的宽度和高度*/
        int width = im.getWidth();
        int height = im.getHeight();
    ...
    */
    ...
    public static void main(String[] args) throws Exception{
    	...
    	String filepath="C:\\Users\\Administrator\\Pictures\\Saved Pictures\\";
    	String filename="shanghai.jpg";
    	BufferedImage im=r.getImage(filepath+filename);
    	r.writeHighQuality(r.zoomImage(im,0.2f), filepath+"big_"+filename);//为防止覆盖原图片,加s_区分是压缩以后的图片
    	r.writeHighQuality(r.zoomImage(im,0.05f), filepath+"smal_"+filename);//为防止覆盖原图片,加s_区分是压缩以后的图片
